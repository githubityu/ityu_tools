...
import 'package:ityu_tools/util/extension/image_aspect_ratio.dart';
...
class ImageUtils {
  ...
  static Future<ui.Image> load(String asset,
      {int width = 5, int height = 3}) async {
  ...
  static ImageProvider getImageProvider(String? picUrl,
      {Image? im, int? maxWidth, int? maxHeight}) {
    return CachedNetworkImageProvider(picUrl!,
  ...
  static FutureBuilder getImageFutureBuilder(String picUrl) {
    return FutureBuilder(
      future: getImageByUrl(picUrl).getAspectRatio(),
      builder: (BuildContext context, AsyncSnapshot<dynamic> snapshot) {
        final isShow =
            snapshot.isDone && snapshot.hasData && snapshot.data != null;
        if (isShow) {
          final ratio = (snapshot.data ?? 1);
          return AspectRatio(
            aspectRatio: ratio < 0.65 ? ratio * 1.2 : ratio,
            child: CacheImage(
              path: picUrl,
            ),
          );
